...
function App() {
  ...
  const updateAppointment = async (id, status) => { const result = await updateBookingStatus(id, status.toLowerCase()); if (result.error) return notify(result.error.message); await refreshAdminData(); if (status === 'Completed') setAdminView('jobs'); notify(`Appointment completed. Create the bill from the Completed jobs column.`) }
...
function CustomerView({ view, profile, points, rewards, vehicles, bookings, bills, loading, appointmentsOpen, setView, onBook, onVehicle, onBattery, onClaim, notify }) {
  ...
  function AdminView({ view, bookings, jobs, bills, customers, rewards, rewardClaims, appointmentsOpen, onToggleAppointments, setView, onBill, onReward, onSlot, onWalkin, onPaid, onJob, onAppointment, notify }) {
