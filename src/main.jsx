...
function App() {
  ...
  const refreshAdminData = async () => {
    ...
    setJobs(data.jobs.map(job => ({ id: job.id, bookingId: job.booking_id, customerId: job.customer_id, customer: job.profiles?.full_name || 'Customer', phone: job.profiles?.phone, referredBy: job.profiles?.referred_by, vehicle: job.vehicles?.model_name || 'Vehicle', issue: job.problem, status: job.status[0].toUpperCase() + job.status.slice(1), time: new Date(job.created_at).toLocaleDateString('en-IN') })))
