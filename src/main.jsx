...
import { claimReward, createBill, createBooking, createReward, createTimeSlot, createVehicle, createWalkinRequest, getAdminData, getCustomerData, getProfile, markBillPaid, normalizePhone, signInWithPhone, signUpWithPhone, supabase, updateBookingStatus, updateJobStatus, updateTimeSlot, updateWorkshopStatus } from './lib/supabase'
import './styles.css'
...
function App() {
  ...
  const refreshAdminData = async () => {
    ...
    setCustomers(data.customers); setRewards(data.rewards); setRewardClaims(data.claims); setAppointmentsOpen(data.appointmentsOpen)
  }
  ...
  return <div className="app-shell">
    ...
    {showBill && <Modal title="Create bill" onClose={() => { setShowBill(false); setBillJob(null) }}><form onSubmit={async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const job = billJob || jobs.find(item => item.id === form.get('jobId')); const parts = Number(form.get('parts') || 0); const service = Number(form.get('service') || 0); const scanning = Number(form.get('scanning') || 0); const total = parts + service + scanning; const result = await createBill({ job_id: job?.id || form.get('jobId'), customer_id: job?.customerId, work_done: form.get('workDone') || 'Service completed', spare_parts: parts, service_charge: service, scanning_cost: scanning }); if (result.error) return notify(result.error.message); await refreshAdminData(); setShowBill(false); setBillJob(null); setAdminView('bills'); notify(`Bill created. Customer earns ${Math.floor(total / 100) * 10} points${total >= 500 && job?.referredBy ? ' + referral bonus: 200 for referrer and 100 for customer' : ''}.`) }}><div className="bill-customer"><strong>{billJob?.customer || 'Select completed job'}</strong>{billJob?.phone && <a href={`tel:${billJob.phone}`}>{billJob.phone}</a>}{billJob?.referredBy && <small>Referred customer: referral bonus applies at ₹500+</small>}</div>{!billJob && <Field label="Completed job"><select name="jobId" required>{jobs.filter(job => job.status === 'Completed').map(job => <option value={job.id} key={job.id}>{job.customer} · {job.vehicle}</option>)}</select></Field>}<Field label="Work done (optional)"><textarea name="workDone" placeholder="Describe the repair completed" /></Field><div className="form-grid"><Field label="Spare parts (optional)"><input name="parts" type="number" min="0" placeholder="0" onInput={event => setBillDraft({ ...billDraft, parts: Number(event.currentTarget.value || 0) })} /></Field><Field label="Service charge (optional)"><input name="service" type="number" min="0" placeholder="0" onInput={event => setBillDraft({ ...billDraft, service: Number(event.currentTarget.value || 0) })} /></Field></div><Field label="Scanning cost (optional)"><input name="scanning" type="number" min="0" placeholder="0" onInput={event => setBillDraft({ ...billDraft, scanning: Number(event.currentTarget.value || 0) })} /></Field><div className="points-preview">Total: <strong>₹{(billDraft.parts + billDraft.service + billDraft.scanning).toLocaleString()}</strong> · Points after payment: <strong>{Math.floor((billDraft.parts + billDraft.service + billDraft.scanning) / 100) * 10}</strong>{billJob?.referredBy && billDraft.parts + billDraft.service + billDraft.scanning >= 500 && <small> · Referral bonus: +200 referrer, +100 customer</small>}</div><FormActions onCancel={() => { setShowBill(false); setBillJob(null) }} submit="Generate bill" /></form></Modal>}
